...
// Some sources (CrossRef, CORE, IEEE) index the front/back matter of books and
// large reports (e.g. the IPCC "Climate Change" volumes) as if each were a
// paper: "Cover", "Preface", "Annex", "Index"... These carry no real content,
// usually have no authors and no abstract, and slip past the keyword filter
// because the VENUE name contains the search terms. Drop them.
const JUNK_TITLES = new Set([
  'cover','front cover','back cover','inside cover','preface','foreword','forward',
  'contents','table of contents','list of contents','index','subject index','author index',
  'name index','annex','annexes','appendix','appendices','editorial','editorial board',
  'acknowledgement','acknowledgements','acknowledgment','acknowledgments','frontmatter',
  'front matter','back matter','title page','half title page','copyright','copyright page',
  'errata','erratum','corrigendum','corrigenda','list of contributors','notes on contributors',
  'list of figures','list of tables','list of abbreviations','abbreviations','glossary',
  'references','bibliography','masthead','colophon','about the authors','about the cover',
  'about the authors','issue information','contributors','frontispiece','dedication',
  'introduction','summary for policymakers','executive summary'
]);
function isJunkTitle(title){
  if(!title) return true;
  let t = String(title).toLowerCase().replace(/\s+/g,' ').trim().replace(/[.:;,]+$/,'').trim();
  if(!t) return true;
  if(JUNK_TITLES.has(t)) return true;
  // "annex a", "appendix 1", "chapter 3", "part ii", "section 2", "table 4"...
  if(/^(annex|annexe|appendix|chapter|part|section|volume|table|figure|plate)\s+[0-9ivxlcdm]{1,5}[a-z]?$/.test(t)) return true;
  return false;
}

// Non-article record types that some sources index as if they were papers:
// book front/back matter, figures/tables ("component"), whole books, container
// records, datasets, peer-review notes, errata, etc. Dropping these removes the
// "weird captions all linking to the same e-book" (e.g. the IPCC Climate Change
// volumes) that leaked in on broad searches like "climate change".
const JUNK_DOC_TYPES = new Set([
  'paratext','component','dataset','other','peer-review','grant','erratum',
  'editorial','reference-entry','libguides','supplementary-materials',
  'journal','journal-issue','journal-volume','book','book-set','book-series',
  'book-track','book-part','edited-book','monograph','reference-book',
...
// Naming the COUNTRY itself (strongest signal) vs. naming an unambiguously
// Pakistani place (Lahore, Karachi, Sindh, Indus basin...). Kept separate so a
// title that names the country is trusted even in a comparative study, while a
// place-only match is held to the conflicting-country guard below.
const PK_COUNTRY_TERMS = ['pakistan', 'pakistani'];
const PK_PLACE_TERMS = UNAMBIGUOUS_PK_TERMS.filter(t => !PK_COUNTRY_TERMS.includes(t));
...
function pakistanConfidence(paper){
  ...
  const titleConflict = textHasAny(title, CONFLICTING_COUNTRY_TERMS);
  const anyConflict = textHasAny(fullText, CONFLICTING_COUNTRY_TERMS);

  // 1) The TITLE names the country Pakistan → trust it, even in a comparative
  //    study that also names other countries (authors title papers after their
  //    real study area).
  if(textHasAny(title, PK_COUNTRY_TERMS)){
    return paper.verifiedPk ? "verified" : "keyword";
  }

  // 2) The TITLE names an unambiguously Pakistani place (Lahore, Karachi, Sindh,
  //    Indus basin...) → keep, UNLESS the title itself also names a conflicting
  //    country. That title-level guard is what stops a transboundary/comparative
  //    paper like "...Indus basin across northwest India..." from passing just
  //    because it also says "Indus". An abstract-only comparison does not
  //    disqualify a genuine title-place match.
  if(textHasAny(title, PK_PLACE_TERMS) && !titleConflict){
    return paper.verifiedPk ? "verified" : "keyword";
  }

  // 3) Pakistan (country or place) is named only in the abstract/venue, not the
  //    title → keep ONLY if NO conflicting country is mentioned anywhere. A bare
  //    abstract mention alongside India/Iran/China/etc. is almost always a
  //    regional or comparative study whose real focus is elsewhere. This is the
  //    case that let "Long-term groundwater variations in Northwest India"
  //    through before (its abstract mentions the Indus basin).
  if(textHasAny(fullText, UNAMBIGUOUS_PK_TERMS) && !anyConflict){
    return paper.verifiedPk ? "verified" : "keyword";
  }

  // Pakistan not credibly the study area (or a bare "Punjab"/"Balochistan" alone,
  // which is shared with India/Iran) → reject.
  return null;
}
...
function matchesTopic(paper, rawKw){
  ...
  // Match keywords against the paper's OWN title + abstract only — NOT the
  // venue/journal name. Otherwise a whole book or report whose title contains
  // the search words (e.g. the IPCC volume "Climate Change 2014") drags in all
  // of its sections/figures/front-matter as if each were a matching paper, even
  // though those sections' own titles have nothing to do with the query.
...
function buildRequests(state){
  ...
  if(state.crHasMore){
    let url = `https://api.crossref.org/works?query=${encodeURIComponent(kwq)}&rows=${PAGE_CR}&offset=${state.crOffset}&select=title,author,published,DOI,container-title,URL,abstract,is-referenced-by-count,type`;
    if(year){
  ...
  if(state.oaHasMore){
    const oaSelect = 'id,title,authorships,publication_year,primary_location,cited_by_count,open_access,ids,abstract_inverted_index,type';
    let url = `https://api.openalex.org/works?search=${encodeURIComponent(kwq)}&select=${oaSelect}&per-page=${PAGE_OA}&page=${state.oaPage}&mailto=grips@app`;
...
async function enrichUnpaywall(papers){
  ...
  const results = await Promise.allSettled(candidates.map(p =>
    fetchWithTimeout(`https://api.unpaywall.org/v2/${encodeURIComponent(p.doi)}?email=grips.app@gmail.com`, undefined, UNPAYWALL_TIMEOUT_MS).then(r => r.ok ? r.json() : null)
  ));
...
async function fetchBatch(state, filters, blobStore){
  ...
  if(ok('cr')){
    ...
    crpapers = items.map(it => {
      ...
      return {
        ...
        url: it.URL, doi: it.DOI, openAccess: false, source: 'CrossRef', verifiedPk: false, docType: (it.type || '').toLowerCase()
      };
    });
  ...
  if(ok('oa')){
    ...
    oapapers = items.map(it => {
      ...
      return {
        ...
        doi, openAccess: !!(it.open_access && it.open_access.is_oa), source: 'OpenAlex', verifiedPk, docType: (it.type || '').toLowerCase()
      };
    });
    state.oaPage += 1;
  ...
  [...oapapers, ...s2papers, ...crpapers, ...arxivpapers, ...springerpapers, ...corepapers, ...elsevierpapers, ...ieeepapers, ...nasaAdsPapers].forEach(p => {
    if(!p.title) return;
    if(isJunkTitle(p.title)) return;
    if(p.docType && JUNK_DOC_TYPES.has(p.docType)) return;
...
module.exports = {
  ...
  pakistanConfidence, isJunkTitle,
